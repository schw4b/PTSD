...
## Load data
```{r} 
d = read.spss(file.path(PATH_DATA, 'PTSD_Skalen_1 06 2017_GESAMT_LS.sav'), 
              to.data.frame = T)
assert(nrow(d) == N)
...
d$id[which(d$schwierigesEreignis=="4")] # 4 bedeutet 
```
...
fit = lm(PTSS_Skala ~ cope_problfoc_active + cope_dysfunct_subst +
           cope_dysfunct_avoid + TraumaArbeit + TraumaVorBeruf +
           SWE_Skala + dauer + geschlecht, data=d.g1_san)
vif(fit) > 4
summary(fit)
```
### Feuerwehr
...
fit = lm(PTSS_Skala ~ cope_problfoc_active + cope_dysfunct_subst +
           cope_dysfunct_avoid + TraumaArbeit + TraumaVorBeruf +
           SWE_Skala + dauer + geschlecht, data=d.g2_feu)
vif(fit) > 4
summary(fit)
```
...
fit = lm(PTSS_Skala ~ cope_problfoc_active + cope_dysfunct_subst +
           cope_dysfunct_avoid + TraumaArbeit + TraumaVorBeruf +
           SWE_Skala + dauer + geschlecht, data=d.g3_pol)
vif(fit) > 4
summary(fit)
```
### Spitalnotfall
...
fit = lm(PTSS_Skala ~ cope_problfoc_active + cope_dysfunct_subst +
           cope_dysfunct_avoid + TraumaArbeit + TraumaVorBeruf +
           SWE_Skala + dauer + geschlecht, data=d.g4_spi)
vif(fit) > 4
summary(fit)
```
### Psychiatrie
...
fit = lm(PTSS_Skala ~ cope_problfoc_active + cope_dysfunct_subst +
           cope_dysfunct_avoid + TraumaArbeit + TraumaVorBeruf +
           SWE_Skala + dauer + geschlecht, data=d.g5_psy)
vif(fit) > 4
summary(fit)
```
...
model = 'Belastung =~ BSCL_PST + BSCL_GSI + GHQ_Psy + GHQ_Emot
         dysfunctional =~ cope_dysfunct_subst +
                          cope_dysfunct_avoid
         dysfunctional ~ SWE_Skala + TraumaVorBeruf
...
semPaths(fit, what = "std", layout = "tree", title = FALSE,  style = "LISREL",
         filetype="png", filename=file.path(PATH_FIGURES,"SEM0-all"))
...
```{r}
# outlier
d.g1_san_rm = d.g1_san[-which.max(d.g1_san$PTSS_Skala),]

fit = sem(model=model, data=d.g1_san_rm)
summary(fit, fit.measures=TRUE)

semPaths(fit, what = "std", layout = "tree", title = FALSE,  style = "LISREL",
         filetype="png", filename=file.path(PATH_FIGURES,"SEM1-San"))
```
...
semPaths(fit, what = "std", layout = "tree", title = FALSE,  style = "LISREL",
         filetype="png", filename=file.path(PATH_FIGURES,"SEM2-Feu"))
```
...
semPaths(fit, what = "std", layout = "tree", title = FALSE,  style = "LISREL",
         filetype="png", filename=file.path(PATH_FIGURES,"SEM3-Pol"))
```
...
semPaths(fit, what = "std", layout = "tree", title = FALSE,  style = "LISREL",
         filetype="png", filename=file.path(PATH_FIGURES,"SEM4-Spi"))
```
...
semPaths(fit, what = "std", layout = "tree", title = FALSE,  style = "LISREL",
         filetype="png", filename=file.path(PATH_FIGURES,"SEM5-Psy"))
```